          ...
          <div className="tx-content">
            ...
            {txHash && (
              <div className="tx-hash-box font-mono">
                ...
                <span className="text-secondary font-bold">{truncate(txHash)}</span>
                {explorerUrl && (
                  <a href={`${explorerUrl}/tx/${txHash}`} target="_blank" rel="noopener noreferrer" className="ml-2 p-1 hover:text-primary transition-colors">
      ...
      case "success":
        return (
          <div className="tx-content">
            ...
            <h2 className="tx-title">Stream Active</h2>
            <p className="tx-description text-on-surface-variant/80">
              You have successfully deployed a trustless salary stream. Assets are now locked in the verified contract.
            </p>
            {streamAddress && (
              <div className="tx-hash-box font-mono mb-4 text-sm bg-surface-container-high rounded-xl p-3 flex justify-between items-center w-full">
                <div className="flex flex-col items-start">
                   <span className="text-[10px] opacity-40 uppercase font-bold tracking-widest mb-1">Contract Address</span>
                   <span className="text-secondary font-bold">{truncate(streamAddress)}</span>
                </div>
                <button 
                  className="p-2 hover:bg-secondary/10 rounded-lg transition-colors"
                  onClick={() => navigator.clipboard.writeText(streamAddress)}
                  title="Copy Address"
                >
            ...
            <div className="tx-actions">
              ...
              {explorerUrl && streamAddress && (
                <a href={`${explorerUrl}/address/${streamAddress}`} target="_blank" rel="noopener noreferrer" className="tx-link-secondary">View on Etherscan</a>
              )}
            </div>
          </div>
        )
